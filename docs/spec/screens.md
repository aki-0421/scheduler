...
- `/` は server redirect を static export せず、通常の HTML fallback から document replacement で `/projects/` を開く。Tauri の main window は `/projects/` を直接開く。
- Tauri の production custom protocol では、Next.js App Router の RSC client navigation を使わない。内部 link は `components/app-link.tsx`、保存・削除後などの programmatic navigation は `lib/navigation.ts` を使い、対象 route を末尾 `/` 付きの独立 HTML document へ正規化して full document navigation する。`next/link`、`useRouter().push`、`useRouter().replace` を内部画面遷移へ直接使わない。この制約は production Tauri で RSC `.txt` payload が document として表示される [Next.js issue #48642](https://github.com/vercel/next.js/issues/48642) を回避するためのものである。
- full document navigation でも theme をまたいだ flash を起こさないよう、全 HTML document は `public/theme-init.js` を parser-blocking な same-origin script として `<head>` 内で実行する。script は互換 storage key `codex-scheduler-theme` の `light` / `dark` を優先し、未保存時は OS の `prefers-color-scheme` を使って、body parse と first paint より前に root の `dark` class と `color-scheme` を同期する。hydration 後も同じ runtime を使って OS theme の change event を追従し、保存済み override がある間は OS change を反映せず、controller の unmount 時に listener を解除する。storage が利用不能、または保存値が `light` / `dark` 以外の場合は OS theme へ fallback する。CSP は `script-src 'self'` を維持し、theme 初期化のために `'unsafe-inline'` を追加しない。
- desktop production build は、上記すべての route output が存在し、UTF-8 の HTML document として開始し、error marker を含まず、route 固有の marker を含むことを bundle 作成前に検証する。あわせて TypeScript AST で runtime source を検査する。`next/link` / `next/router` は import alias、namespace、`require()`、dynamic `import()` を含めて拒否し、`next/navigation` は `usePathname` / `useSearchParams` の read-only allowlist だけを許可して `useRouter`、`redirect`、`permanentRedirect` と解決不能な namespace / dynamic access を拒否する。History API は URL を省略するか query / hash だけを静的に更新する場合に限り、別画面 path と解決不能な URL を拒否する。raw `<a>` は `AppLink` 自身と静的な external / `mailto:` / `tel:` / hash link だけを原則許可し、const、conditional、template、spread を解決して内部または不明な href を拒否する。ReactMarkdown の URL transform 後の外部 link renderer は scanner 内の理由付き file allowlist で例外化する。`AppLink` が正規化 helper の結果を anchor の `href` に設定すること、route map が全画面を持つこと、programmatic helper が正規化結果を `assign` / `replace` へ渡すことも AST で検証する。さらに theme 初期化 script が全画面の `<head>` にあり、export source、`color-scheme` metadata / CSS、CSP と整合することを検証する。1画面でも違反した場合は build を失敗させる。
